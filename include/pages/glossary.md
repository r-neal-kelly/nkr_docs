...
A `function` is an [entity][entity] that can never be used as the [primary entity][primary entity] in an [entity group][entity group]. Unlike most other entities, functions do not have a [postfix][postfix] in their [labels][label], however each word in their name is capitalized to distinguish functions from other entities. Instead of using functions as primary entities, the [public namespaces][public namespace] or other entities that contain functions should be used. This is to avoid [file][file] label collisions on disk, which may not respect the capitalization of labels. All function [labels][label] have no [postfix][postfix].
